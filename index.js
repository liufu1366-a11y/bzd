...
async function installBuiltinMechagirlWorldbook({ overwrite = false } = {}) {
  ...
  for (const src of (preset.entries || [])) {
    ...
    entry.key = Array.isArray(src.key) ? src.key : [];
    entry.keysecondary = Array.isArray(src.keysecondary) ? src.keysecondary : [];
    entry.content = src.content || "";
    entry.constant = Boolean(src.constant);
    entry.selective = Boolean(src.selective);
    entry.order = Number(src.order) || 100;
    entry.position = Number.isFinite(Number(src.position)) ? Number(src.position) : entry.position;
    entry.disable = false;
  }
