...
function openEditor() {
  ...
  if(storySelect){
    ...
    storySelect.onchange=()=>{
      ...
      const wc=$id("xjn-word-count");if(wc)wc.textContent=`${ta.value.length.toLocaleString()} 字 · 已保存`;
    };
  }
  ...
  // 先绑定核心按钮，再做任何可选状态检测；这样状态模块出错也不会让整排按钮失效。
  ...
  if(coreStatus){ coreStatus.textContent="按钮已绑定"; coreStatus.classList.add("ok"); }
