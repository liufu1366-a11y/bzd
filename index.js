...
const BUILTIN_MECHAGIRL_WB_URL = new URL("./assets/机械少女（机娘）完整设定_V1.2.worldbook.json", import.meta.url);

async function loadBuiltinMechagirlPreset() {
  const response = await fetch(BUILTIN_MECHAGIRL_WB_URL);
  if (!response.ok) throw new Error(`读取内置世界书失败：HTTP ${response.status}`);
...
async function openBuiltinWorldbookDirectory() {
  try {
    ...
    modal(`机娘 V1.2 词条目录 · ${(preset.entries||[]).length} 条`, `
      <div class="xjn-tip">这里直接读取续写鸡内置的世界书文件，不需要等待酒馆原生世界书编辑器加载。</div>
    ...
    const body = `
      ...
      <div class="xjn-builtin-wb-card">
        <div class="xjn-builtin-wb-title">
          ...
          <span>${builtinInstalled ? "已安装" : "未安装"}</span>
        </div>
        ...
        <div class="xjn-toolbar">
          <button id="xjn-install-mechagirl-wb" class="menu_button primary">
            ${builtinInstalled ? "重新安装 / 更新" : "安装到酒馆世界书"}
          </button>
...
function renderIntegrationStrip(){
  ...
  el.innerHTML=
    ...
    pill("机娘V1.2",st.mechagirlBookActive,
      st.mechagirlBookActive ? "机械少女（机娘）V1.2 世界书已启用" :
      st.mechagirlBookInstalled ? "已安装但未启用" : "尚未安装");
  el.querySelectorAll(".xjn-status-pill").forEach(b=>b.onclick=()=>openDiagnostics());
}
...
function openStoryManager(ta){
  ...
  page.querySelectorAll(".xjn-story-del").forEach(b=>b.onclick=()=>{
    ...
    let l=storyLibrary().filter(x=>x.id!==b.dataset.id);localStorage.removeItem(storyDataKey(b.dataset.id));saveStoryLibrary(l);
...
function openEditor() {
  ...
  screen.innerHTML = `
    ...
    <div class="xjn-editor-actions">
      ...
      <button id="xjn-stories" class="menu_button">故事</button>
  ...
  $id("xjn-stories").onclick = () => openStoryManager(ta);
  ...
  $id("xjn-summary").onclick = async () => {
    const btn=$id("xjn-summary"); if(btn){btn.disabled=true;btn.textContent="总结中…";}
    try {
      editorText = ta.value;
      editorDirty = true;
    ...
    } finally { if(btn){btn.disabled=false;btn.textContent="总结正文";} }
  };
