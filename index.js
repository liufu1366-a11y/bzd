...
function openLocalWorldbookEntryEditor(index, parentPage=null){
  ...
  const page=modal(`编辑词条 ${index+1}`,`
    <div class="xjn-lite-form">
      ...
      <div class="xjn-toolbar"><button type="button" id="xjn-one-wb-save" class="menu_button primary">保存词条</button></div>
    </div>`,"xjn-wide");
...
function openLocalWorldbookEditor(){
  ...
  const page=modal("机娘 V1.2 · 世界书",`
    ...
    <div class="xjn-toolbar">
      ...
      <button type="button" id="xjn-local-wb-reset" class="menu_button">恢复 V1.2 原版</button>
  ...
  try {
    ...
    const body = `
      ...
      <div class="xjn-builtin-wb-card">
        ...
        <div class="xjn-toolbar">
          <button type="button" id="xjn-install-mechagirl-wb" class="menu_button primary">
          ...
          <button type="button" id="xjn-builtin-wb-directory" class="menu_button">词条目录</button>
...
function openAppearance(screen) {
  ...
  const w = modal("背景 / 字体", `
    ...
    <div class="xjn-toolbar">
      <button type="button" id="xjn-ap-save" class="menu_button primary">应用并保存</button>
    </div>
  `,"xjn-wide");
...
function renderIntegrationStrip(){
  ...
  try{
    ...
    const pill=(name,on,tip)=>`<span class="xjn-status-pill ${on?"ok":"off"}" title="${esc(tip)}"><span>${on?"●":"○"}</span> ${esc(name)}</span>`;
    el.innerHTML=
      pill("动态Prompt",st.dynamicPrompt,st.dynamicPrompt?"已启用":"当前关闭")+
      pill("事件监听",st.eventBridge,st.eventBridge?"酒馆事件桥已就绪":"事件桥未就绪")+
      pill("原生世界书",st.nativeWorldbook,st.nativeWorldbook?"已连接 World Info API":"未连接 World Info API")+
      pill("机娘V1.2",isLocalMechagirlEnabled(),isLocalMechagirlEnabled()?"轻量世界书已启用":"轻量世界书未启用");
  }catch(e){
    console.warn("[XJN] 状态条渲染失败，已忽略：",e);
    el.textContent="状态检测已降级（不影响编辑器功能）";
  }
}
...
function openDatabaseManager(){
  ...
  const body=`
    ...
    <div class="xjn-toolbar">
      <button type="button" id="xjn-db-build" class="menu_button primary">${idx?"重新建立数据库":"建立数据库"}</button>
...
function openStoryManager(ta){
  ...
  const page=modal("故事 / 续写项目",`
    ...
    <div class="xjn-toolbar"><button type="button" id="xjn-story-new" class="menu_button primary">新建故事</button></div>
...
function updateNewContentDivider(){
  const el=$id("xjn-new-content-divider");
  if(!el)return;
  el.hidden=!newContentStart;
}

function updateEditorCountFromTextarea(saved=false){
  const ta=$id("xjn-editor-text"), wc=$id("xjn-word-count");
  if(!ta||!wc)return;
  wc.textContent=`${ta.value.length.toLocaleString()} 字 · ${saved?"已保存":"未保存"}`;
  const live=$id("xjn-live-count");
  if(live)live.textContent=`｜编辑框实时：${ta.value.length.toLocaleString()}`;
  updateNewContentDivider();
}

function renderEditor() {
  // textarea 是唯一实时正文源；这里绝不读取旧 editorText 覆盖字数。
  updateEditorCountFromTextarea(!editorDirty);
}
function updateBusy() {
...
function runFastToolbarAction(button, label, fn) {
  ...
  button.onclick = async (ev) => {
    ...
    try {
      ...
      // 不 blur、不 pointerup、不依赖其它模块，Android WebView 走最普通 click。
      await Promise.resolve();
      await fn();
    } catch (e) {
      console.error("[XJN button]", button.id, e);
      try { toastr.error(String(e?.message || e), "功能打开失败"); } catch {}
    } finally {
...
function openEditor() {
  ...
  screen.innerHTML = `
    <div class="xjn-editor-topbar">
      <button type="button" id="xjn-editor-back" class="menu_button">返回</button>
      <div class="xjn-editor-title-wrap">
        <b>续写鸡 NEXT <small class="xjn-version">v${VERSION}</small></b>
        <small id="xjn-story-name"></small>
        <small id="xjn-word-count">0 字</small>
        <small id="xjn-core-status" class="xjn-core-status">初始化中</small>
      </div>
      <button type="button" id="xjn-save" class="menu_button">保存</button>
    </div>

    <div class="xjn-editor-actions">
      <button type="button" id="xjn-generate" class="menu_button primary">开始续写</button>
      <select id="xjn-story-select" class="text_pole xjn-story-select"></select>
      <button type="button" id="xjn-story-new-quick" class="menu_button">＋故事</button>
      <label class="menu_button xjn-file-button">导入TXT<input id="xjn-import-txt" type="file" accept=".txt,text/plain" hidden></label>
      <button type="button" id="xjn-chapters" class="menu_button">章节定位</button>
      <button type="button" id="xjn-appearance" class="menu_button">背景/字体</button>
      <button type="button" id="xjn-local-worldbook" class="menu_button">世界书</button>
      <button type="button" id="xjn-database" class="menu_button">数据库</button>
      <button type="button" id="xjn-jump-new" class="menu_button">新内容</button>
      <button type="button" id="xjn-editor-size" class="menu_button">正文全屏</button>
      <button type="button" id="xjn-lite-mode" class="menu_button">轻量模式</button>
      <button type="button" id="xjn-summary" class="menu_button">总结正文</button>
      <button type="button" id="xjn-export" class="menu_button">导出TXT</button>
      <button type="button" id="xjn-restore-import" class="menu_button">恢复导入原文</button>
    ...
    <div class="xjn-token-policy">分层上下文：最近正文 ≤12,000 字符；长期记忆 ≤4,200；世界书 ≤5,000；数据库旧原文 ≤7,500。数据库与长期记忆会自动减少重复。 <span id="xjn-live-count"></span></div>
    ...
    <div id="xjn-new-content-divider" class="xjn-new-content-divider" hidden>
      <span>以下为本次新生成内容</span>
    </div>
  ...
  // v1.0.21：核心按钮必须先绑定。故事库/外观/状态等任何数据异常都不能阻断主界面。
  const ta = $id("xjn-editor-text");
  const coreStatus=$id("xjn-core-status");
  if(coreStatus){coreStatus.textContent="核心按钮已绑定";coreStatus.classList.add("ok");}

  // 先放入正文；失败也只显示空文本，不终止初始化。
  try { ta.value = editorDisplayText(); }
  catch(e) {
    console.error("[XJN] editorDisplayText failed",e);
    ta.value = String(editorText||"");
  }
  try { updateEditorCountFromTextarea(!editorDirty); } catch {}
  ...
  // 故事库单独初始化；旧数据损坏时自动退回默认故事，不影响其它按钮。
  const storyNameEl=$id("xjn-story-name");
  const storySelect=$id("xjn-story-select");
  try{
    let list=storyLibrary();
    if(!Array.isArray(list) || !list.length){
      list=[{id:"default",name:"默认故事",updatedAt:Date.now()}];
      saveStoryLibrary(list);
      localStorage.setItem(STORY_SELECTED_KEY,"default");
      if(!getStoryData("default")) setStoryData("default",{editor:String(editorText||""),importedBook:null});
    }
    let sid=selectedStoryId();
    if(!list.some(s=>s.id===sid)){
      sid=list[0].id;
      localStorage.setItem(STORY_SELECTED_KEY,sid);
    }
    const cur=list.find(s=>s.id===sid)||list[0];
    if(storyNameEl)storyNameEl.textContent=cur?.name||"默认故事";
    if(storySelect){
      storySelect.innerHTML=list.map(s=>`<option value="${esc(s.id)}" ${s.id===sid?"selected":""}>${esc(s.name||"未命名故事")}</option>`).join("");
      storySelect.onchange=()=>{
        try{
          ...
          loadEditor();
          ta.value=editorText||"";
          const selected=list.find(s=>s.id===storySelect.value);
          if(storyNameEl)storyNameEl.textContent=selected?.name||"默认故事";
          updateEditorCountFromTextarea(true);
        }catch(e){
          console.error("[XJN story switch]",e);
          toastr.error("故事切换失败，但其它功能仍可使用","续写鸡 NEXT");
        }
      };
    }
  }catch(e){
    console.error("[XJN story init degraded]",e);
    if(storyNameEl)storyNameEl.textContent="故事库已降级";
    if(storySelect)storySelect.innerHTML='<option value="">故事库异常</option>';
  }
  ...
  if(quickNew)quickNew.onclick=()=>{
    try{
      const name=prompt("新故事名称：","新故事");if(!name)return;
      const id=`story_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;
      let l=storyLibrary(); if(!Array.isArray(l))l=[];
      l.push({id,name:name.trim()||"新故事",updatedAt:Date.now()});saveStoryLibrary(l);
      setStoryData(id,{editor:"",importedBook:null});localStorage.setItem(STORY_SELECTED_KEY,id);
      editorText="";editorDirty=false;ta.value="";
      if(storyNameEl)storyNameEl.textContent=name.trim()||"新故事";
      if(storySelect)storySelect.innerHTML=l.map(s=>`<option value="${esc(s.id)}" ${s.id===id?"selected":""}>${esc(s.name||"未命名故事")}</option>`).join("");
      updateEditorCountFromTextarea(true);
    }catch(e){
      console.error("[XJN new story]",e);
      toastr.error("新建故事失败","续写鸡 NEXT");
    }
  };

  const targetInput = $id("xjn-editor-target-chars");
  if (targetInput) {
    try { targetInput.value = getSettings()?.targetChars || 2000; }
    catch { targetInput.value = 2000; }
  }
  // 外观属于可选功能，任何主题/字体错误都不能阻断后续按钮绑定。
  try { applyEditorAppearance(screen); }
  catch (e) {
    console.warn("[XJN] 外观初始化失败，已降级，不影响按钮：", e);
    screen.style.color = "inherit";
  }

  const importInput=$id("xjn-import-txt");
  if(importInput) importInput.onchange = async (ev) => {
    ...
    try {
      ...
      ta.value = editorText;
      editorDirty = true;
      updateEditorCountFromTextarea(false);
      ta.scrollTop = ta.scrollHeight;
      renderEditor();
    } catch (e) {
      console.error("[XJN import]", e);
      toastr.error(String(e?.message || e), "导入失败");
    } finally {
      ev.target.value = "";
    }
  };
  const jumpNew=$id("xjn-jump-new");
  if(jumpNew)jumpNew.onclick=()=>{
    const pos=Math.max(0, Number(newContentStart)||0);
    if(!pos){toastr.info("当前还没有本次新生成内容","续写鸡 NEXT");return;}
    ta.focus({preventScroll:true});
    ta.setSelectionRange(pos,pos);
    const ratio=pos/Math.max(1,ta.value.length);
    ta.scrollTop=Math.max(0,ratio*(ta.scrollHeight-ta.clientHeight)-80);
  };
  const sizeBtn=$id("xjn-editor-size");
  if(sizeBtn)sizeBtn.onclick=()=>{
    screen.classList.toggle("xjn-reading-focus");
    sizeBtn.textContent=screen.classList.contains("xjn-reading-focus")?"退出全屏":"正文全屏";
    setTimeout(()=>{ta.scrollTop=ta.scrollTop;},0);
  };
  ...
  ta.addEventListener("input", () => {
    editorDirty = true;
    // 正文以 textarea 为实时真源；不要每输入一次复制整篇到 editorText。
    if(xjnCountRaf)return;
    xjnCountRaf=requestAnimationFrame(()=>{
      xjnCountRaf=0;
      const wc=$id("xjn-word-count");
      if(wc)wc.textContent=`${ta.value.length.toLocaleString()} 字 · 未保存`;
    });
  });
  ...
  updateEditorCountFromTextarea(!editorDirty);
  // Android 输入法/粘贴有时不会按普通 input 路径触发，额外监听这些事件。
  const syncCount=()=>requestAnimationFrame(()=>updateEditorCountFromTextarea(!editorDirty));
  ta.addEventListener("paste",()=>setTimeout(syncCount,0));
  ta.addEventListener("change",syncCount);
  ta.addEventListener("compositionend",syncCount);
  try { renderEditor(); } catch(e) { console.warn("[XJN] renderEditor 降级",e); }
  try { updateBusy(); } catch(e) { console.warn("[XJN] updateBusy 降级",e); }
  try { renderIntegrationStrip(); } catch(e) { console.warn("[XJN] integration 降级",e); }

  // 最后保险：核心按钮事件委托。即使某个 onclick 被酒馆/主题覆盖，也能从编辑器根节点接住点击。
  screen.addEventListener("click", (ev) => {
    const b=ev.target?.closest?.("button");
    if(!b || b.dataset.xjnDelegated==="1")return;
    const id=b.id;
    const map={
      "xjn-chapters":()=>openChapterNavigator(screen,ta),
      "xjn-appearance":()=>openAppearance(screen),
      "xjn-local-worldbook":()=>openLocalWorldbookEditor(),
      "xjn-database":()=>openDatabaseManager()
    };
    if(!map[id])return;
    // 正常 onclick 已存在时不重复执行；只在 onclick 缺失时兜底。
    if(typeof b.onclick==="function")return;
    b.dataset.xjnDelegated="1";
    try{ map[id](); }catch(e){ console.error("[XJN delegated]",e); }
    finally{ delete b.dataset.xjnDelegated; }
  });
  if(coreStatus){ coreStatus.textContent="初始化完成"; coreStatus.classList.add("ok"); }
...
function openWorldbook() {
  let list=worldbook();
  const w=modal("内置世界书", `
    <div class="xjn-toolbar"><button type="button" id="xjn-wb-add" class="menu_button primary">新增条目</button><button type="button" id="xjn-wb-export" class="menu_button">导出JSON</button><label class="menu_button">导入JSON<input id="xjn-wb-import" type="file" accept=".json" hidden></label></div>
...
function openMemory() {
  ...
  const w=modal("长期记忆 / 总结库", `
    <div class="xjn-toolbar"><button type="button" id="xjn-mem-add" class="menu_button primary">新增记忆</button><button type="button" id="xjn-mem-export" class="menu_button">导出JSON</button></div>
...
function openRP() {
  ...
  const w = modal("动态故事状态", `
    ...
    <div class="xjn-toolbar">
      <button type="button" id="xjn-rp-save" class="menu_button primary">保存状态</button>
      <button type="button" id="xjn-rp-reset" class="menu_button">清空状态</button>
    </div>
  `,"xjn-wide");
...
function openTemplate() {
  ...
  const w=modal("动态 Prompt 模板", `
    ...
    <div class="xjn-toolbar"><button type="button" id="xjn-template-save" class="menu_button primary">保存模板</button><button type="button" id="xjn-template-reset" class="menu_button">恢复默认</button></div>
  `,"xjn-wide");
...
function openDiagnostics(){
  const w=modal("系统自检 / 事件日志",`<div id="xjn-diag-body"></div><div class="xjn-toolbar"><button type="button" id="xjn-log-clear" class="menu_button">清空日志</button></div>`,"xjn-wide");
  ...
  return `<div class="xjn-settings"><div class="inline-drawer">
    ...
    <div class="inline-drawer-content">
      <div class="xjn-settings-buttons">
        <button type="button" id="xjn-open-editor" class="menu_button primary">打开长篇编辑器</button>
        <button type="button" id="xjn-open-worldbook" class="menu_button">世界书</button>
        <button type="button" id="xjn-open-memory" class="menu_button">长期记忆</button>
        <button type="button" id="xjn-open-rp" class="menu_button">动态状态</button>
        <button type="button" id="xjn-open-template" class="menu_button">动态Prompt</button>
        <button type="button" id="xjn-open-diag" class="menu_button">自检 / 日志</button>
      </div>
      <hr class="sysHR">
      ...
      <hr class="sysHR">
      <div class="xjn-settings-buttons"><button type="button" id="xjn-export-all" class="menu_button">导出完整备份</button><label class="menu_button">导入完整备份<input id="xjn-import-all" type="file" accept=".json" hidden></label></div>
    </div></div></div>`;
}
